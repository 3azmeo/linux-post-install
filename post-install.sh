...
#  Version: 6.0 (Full Rewrite for Syntax Safety)
# ==============================================================================
...
print_header() {
    ...
    echo -e "${C_BLUE}     3azmeo Post-Install Script v6.0       ${C_RESET}"
    echo -e "${C_BLUE}=============================================${C_RESET}"
    echo
}
...
detect_package_manager() {
    if command -v apt &> /dev/null; then
        ...
        log_error "Could not detect a known package manager. Exiting."
        exit 1
    fi
}
...
setup_security_tool() {
    ...
    case $choice in
        1)
            ...
            if ask_yes_no "Install CrowdSec Firewall Bouncer?"; then
                apt install -y crowdsec-firewall-bouncer-iptables
            fi
            systemctl enable --now crowdsec
            log_success "CrowdSec installed."
            ;;
        2)
...
harden_ssh_server() {
    log_info "Hardening SSH server..."
    sed -i -E 's/^#?PermitRootLogin.*/PermitRootLogin no/' /etc/ssh/sshd_config
    sed -i -E 's/^#?PasswordAuthentication.*/PasswordAuthentication no/' /etc/ssh/sshd_config
    log_success "Disabled root login and password auth."
...
setup_zsh_p10k() {
    if ask_yes_no "Install Oh-My-Zsh + Powerlevel10k theme for user '$TARGET_USER'? ✨"; then
        ...
        log_action "The script will pause. Please follow the on-screen prompts."
        echo
        ...
        echo -e "${C_YELLOW}   Download from: ${C_GREEN}https://github.com/romkatv/powerlevel10k#meslo-nerd-font-patched-for-powerlevel10k"
        echo -e "${C_YELLOW}2. After installing the font, set it in your terminal application (Windows Terminal, iTerm2, etc)."
        echo -e "${C_YELLOW}3. To change the theme again later, run: ${C_GREEN}p10k configure${C_RESET}"
        echo
    fi
}

# --- Optional Components & Finalization ---
install_docker() {
...
configure_nfs_mounts() {
    if ask_yes_no "Setup and mount NFS shares?"; then
        if ! command -v mount.nfs &> /dev/null; then
            log_error "'mount.nfs' not found. Ensure 'nfs-common' is installed."
            return 1
        fi
        while ask_yes_no "Add a new NFS share?"; do
            read -p "Enter remote share path: " remote_share
